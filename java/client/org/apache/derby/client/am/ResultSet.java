...
//        cursor_ != null) {
  ...
  private void updateRowX () throws java.sql.SQLException
  {
    ...
    if (updatedColumns_ == null) return;
  ...
  {
    synchronized (connection_) {
      if (agent_.loggingEnabled()) agent_.logWriter_.traceEntry (this, "deleteRow");
      deleteRowX();
    }
  }
  ...
  private void deleteRowX () throws java.sql.SQLException
  {
    ...
    if (preparedStatementForDelete_ == null) getPreparedStatementForDelete ();
  ...
  {
    synchronized (connection_) {
      if (agent_.loggingEnabled()) agent_.logWriter_.traceEntry (this, "refreshRow");
      refreshRowX();
    }
  }
  ...
  {
    synchronized (connection_) {
      if (agent_.loggingEnabled()) agent_.logWriter_.traceEntry (this, "cancelRowUpdates");
      checkForClosedResultSet();
      if (isOnInsertRow_ || resultSetConcurrency_ == java.sql.ResultSet.CONCUR_READ_ONLY)
        throw new SqlException (agent_.logWriter_, "This method cannot be invoked while the cursor is on the insert " +
      ...
      if (!updateRowCalled_) resetUpdatedColumns ();
    }
  }
  ...
  {
    synchronized (connection_) {
      if (agent_.loggingEnabled()) agent_.logWriter_.traceEntry (this, "moveToInsertRow");
      checkForClosedResultSet ();
  ...
  {
    synchronized (connection_) {
      if (agent_.loggingEnabled()) agent_.logWriter_.traceEntry (this, "moveToCurrentRow");
      checkForClosedResultSet ();
  ...
  {
    if (agent_.loggingEnabled()) agent_.logWriter_.traceExit (this, "getStatement", statement_);
    return statement_;
  }
  ...
  protected void positionToCurrentRowAndUpdate () throws SqlException
  {
    ...
    if (!preparedStatementForUpdate_.openOnServer_ )
      preparedStatementForUpdate_.materialPreparedStatement_.writePrepare_ (
          preparedStatementForUpdate_.sql_,
    ...
    if (chainAutoCommit) connection_.writeCommit ();
    ...
    if (chainAutoCommit) connection_.readCommit ();
    agent_.endReadChain ();
  }
  ...
  protected void positionToCurrentRowAndDelete() throws SqlException
  {
    ...
    if (!preparedStatementForDelete_.openOnServer_ )
      preparedStatementForDelete_.materialPreparedStatement_.writePrepare_ (
          preparedStatementForDelete_.sql_,
  ...
  {
    preparedStatementForUpdate_.materialPreparedStatement_.writeExecute_ (
      preparedStatementForUpdate_.section_,
  ...
  {
    if (isRowsetCursor_)
      preparedStatementForDelete_.materialPreparedStatement_.writeExecute_ (
          preparedStatementForDelete_.section_,
  ...
  {
    if (connection_.autoCommit_) markAutoCommitted();
  }
  ...
  {
    if (sensitivity_ == sensitivity_sensitive_dynamic__) return false;
    else return rowCount_ == -1;
  }

  protected boolean rowCountIsKnown () { return rowCount_ != -1; }
  ...
  private void getPreparedStatementForUpdate () throws java.sql.SQLException
  {
    ...
    preparedStatementForUpdate_ =
      statement_.connection_.preparePositionedUpdateStatement (
        updateString,
    ...
    if (sensitivity_ == sensitivity_sensitive_dynamic__) {
      switch (orientation) {
        case scrollOrientation_next__:
          if (isAfterLast_) return false;
          else return (currentRowInRowset_+1 < rowsReceivedInCurrentRowset_);
        case scrollOrientation_prior__:
          if (isBeforeFirst_) return false;
          else return (currentRowInRowset_-1 >= 0);
  ...
  //    Return to Client - not to be read by the stored procedure only by client
  //    Return to Caller - only calling JSP can read it, not the client
  public byte getRSReturnability() { return rsReturnability_; }

  public void setRSReturnability( byte rsReturnability )
  { // valid return type set it
    ...
    { // unknown return type, set DDM_RETURN_CALLER
...
//------------------------------------------------------------------------------
  ...
  private boolean getPreviousRowset () throws SqlException
  {
    ...
    else currentRowInRowset_ = rowsReceivedInCurrentRowset_-1;
    return true;
  }
  ...
  private boolean getLastRowset (long row) throws SqlException
  {
    ...
    // adjust the cursor positions for sensitive static or insensitive cursors only
    if (sensitivity_ != sensitivity_sensitive_dynamic__) adjustLastRowset (row);
    else
